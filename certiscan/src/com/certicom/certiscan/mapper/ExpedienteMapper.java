...
public interface ExpedienteMapper {
	...
	@Select("select temp_dir, nro_expediente, count(expediente_id) as cantidad from t_expediente where cast(fecha_reg as Date) >= #{fec_ini} and cast(fecha_reg as Date) <= #{fec_fin} and id_estado = 6 group by temp_dir, nro_expediente order by count(expediente_id) desc;")
	public List<Expediente> obtenerCarpetas(Expediente expediente)throws Exception;
...
//			+ "#{observacion},#{edad},#{periodo_pago},#{periodo_vehicular}, #{id_producto})")
	...
	@Select("select * from t_expediente where id_lote = #{id_lote}")
	public List<Expediente> findByIdLote(@Param("id_lote") Integer id_lote) throws Exception;
	...
	@Select("select ex.*, es.descripcion as des_estado  from t_expediente ex inner join t_estado es on ex.id_estado = es.id_estado  where ex.nro_expediente = #{nro_expediente} and ex.id_estado = 2 order by ex.fecha_reg desc limit 1")
	public Expediente findByNroExpedientePrepToDigi(@Param("nro_expediente") String nroexpediente) throws Exception;
