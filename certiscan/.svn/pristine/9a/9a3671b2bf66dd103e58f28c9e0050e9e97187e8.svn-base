...
@ManagedBean(name="reAgruparMB")
@ViewScoped
public class ReAgruparMB extends GenericBeans implements Serializable{
...
//			this.listaExpTracking = this.expedienteTrackingService.listarTrackingxExpediente(expTra.getCod_expediente());
	...
	public void guardar(){
		...
		try {
			...
			Integer max = this.loteServices.maxIdLote();
			
			max = max + 1;
			
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			
			String cod_lote = sdf.format(new Date())+"-"+max.toString();
			
			this.lote.setCod_lote(cod_lote);			
			this.lote.setFecha_registro(new Date());
			this.lote.setCantidad_expedientes(this.cantidad_expedientes);
			this.lote.setEstado("POR ENTREGAR");
			
			this.loteServices.crearLote(this.lote);
			...
			for (int i = 0; i < this.selectedExpediente.length; i++) {
				
				Expediente exp = this.selectedExpediente[i];
				
				exp.setId_estado(Constante.EST_PREPARADO_PARA_ENTREGAR_A_MENSAJERIA);
				exp.setId_lote(max);
				exp.setCod_lote(this.lote.getCod_lote());
				
				this.nro_expediente = exp.getNro_expediente();
				
				System.out.println("NRO EXPEDIENTE " + this.nro_expediente);
				
				listExpGuardar.add(exp);
				
				ExpedienteTracking et = new ExpedienteTracking();
				et.setExpediente_id(exp.getExpediente_id());
				et.setFecha_registro(new Date());
				et.setIdusuario(this.usuarioLogin.getIdusuario());
				et.setIdusuario_registro(this.usuarioLogin.getIdusuario());				
				et.setCondicion(exp.getCondicion());
				et.setFecha_recepcion(exp.getFecha_recepcion());	
				et.setId_oficina(this.usuarioLogin.getId_oficina());
				
				et.setId_estado(Constante.EST_REAGRUPADO);
				...
				et.setId_estado(Constante.EST_PREPARADO_PARA_ENTREGAR_A_MENSAJERIA);
	...
	//			}
				...
				this.listaExpedientes = this.expedienteService.consultaReagruparExpediente(expedienteFilter);
	...
	public void cargarExpedientes(){
		...
		if(this.selectedExpediente == null){
			...
			context.update("dlgDetSeleccion");
			context.execute("PF('dlgSeleccion').show()");
			
		}else{
		
			if(this.selectedExpediente.length == 0){
				...
				this.cantidad_expedientes = this.selectedExpediente.length;
				this.fechaRegistro = new Date();
				
				context.update("dlgDetExpAgr");
				context.execute("PF('dlgExpAgr').show()");
